...
func FileExport(ctx App, res http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		return
	} else if !model.CanRead(&ctx) {
		SendErrorResult(res, ErrPermissionDenied)
		return
	}

	var tmpPath = GetAbsolutePath(TmpPath) + "/export_" + QuickString(10)
	var cmd *exec.Cmd
	...
	if reqMimeType == "text/org" {
		...
		if f, err := os.OpenFile(GetAbsolutePath(ConfigPath+"emacs.el"), os.O_WRONLY|os.O_CREATE|os.O_EXCL, os.ModePerm); err == nil {
		...
		if mimeType == "text/html" {
			cmd = exec.Command(
				...
				"--load", GetAbsolutePath(ConfigPath+"emacs.el"),
