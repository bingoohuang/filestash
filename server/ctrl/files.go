...
func init() {
	...
	cachePath := filepath.Join(GetCurrentDir(), TmpPath)
...
func FileCat(ctx App, res http.ResponseWriter, req *http.Request) {
	...
	if req.Header.Get("range") != "" && needToCreateCache {
		if obj, ok := file.(io.Seeker); ok {
			...
			tmpPath := filepath.Join(GetCurrentDir(), TmpPath, "file_"+QuickString(20)+".dat")
